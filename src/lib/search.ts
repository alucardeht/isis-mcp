...
function log(provider: string, message: string): void {
  console.log(`[Search] [${provider}] ${message}`);
}

function logError(provider: string, error: unknown): void {
...
async function searchDuckDuckGo(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const formatted = searchResults.results.slice(0, maxResults).map((r: any) => ({
        url: r.link || r.url || "",
        title: r.title || "",
...
async function searchSearxngPublic(
  ...
  try {
    ...
    log(
      "searxng-public",
      `Success with ${instance} - found ${formatted.length} results`
    );
    return formatted;
  } catch (error) {
